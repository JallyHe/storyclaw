...
import { existsSync, mkdtempSync, mkdirSync, readdirSync, readFileSync, writeFileSync, copyFileSync, rmSync } from 'fs'
import os from 'os'
...
const EXPECTED_FORMAT_MARKERS: Partial<Record<AgentName, string[]>> = {
  ...
  'story-restructurer': ['#'],                              // Markdown 故事脉络
  'plot-designer': ['#'],                                   // Markdown Synopsis
  'scene-planner': ['#'],                                   // Markdown 分场创作指导
  'scene-to-script': ['【'],                                 // 剧本格式：【场景标题】
  'dialogue-optimizer': ['【'],                              // 剧本格式：【场景标题】
  'plot-to-screenplay': ['【'],                              // 剧本格式：【场景标题】
  'core-strategist': ['#'],                                 // Markdown
